...
\section{The Residual ~3.5$\times$ Factor}

Dividing Wu Q by $10^{8}$ converts it to \emph{genuine SI units}
(K\,m$^{2}$\,kg$^{-1}$\,s$^{-1}$).  The resulting field, when compared
to ERA5's native Ertel PV, shows a spatially-varying residual factor of
$\sim$2--4$\times$ (median $\sim$3.5$\times$).  This residual is
\emph{physical}, not a units problem:

\begin{itemize}
  \item Wu uses a 5-point finite-difference Laplacian on a
        $1.5^{\circ}$ grid; ERA5 computes PV from its native spectral
        representation.
  \item Wu computes $\partial\theta/\partial\Pi$ from 3-point
        centred differences on 8 unevenly-spaced $\sigma$-levels;
        ERA5 has 137 model levels.
  \item The static stability $\partial\theta/\partial p$ is sensitive
        to vertical resolution; coarse differencing systematically
        underestimates it by $\sim$2--3$\times$.
\end{itemize}

The median ratio of $Q_{\text{Wu}}\!\times\!10^{-8}$ to ERA5 PV can
be reported as a diagnostic of the discretisation bias, but it must
\emph{not} be used to scale the Wu PV field.  The correct conversion
is the analytic $10^{-8}$.
...
\begin{warnbox}[COEF uses $g=9.81$, not $g=9.80665$]
