...
C	READ (5,'(a)')F(1)
	...
	READ (5,'(a)')(F(I),I=2,3)
C        F(2)='theta_q_out_mean'
C	F(3)='h_psi_out_mean'

C	print*, 'this file contains THb, THt and interior Q (PV):', F(2)
C	print*, 'this file contains H (geopotential) and Psi:', F(3)
...
c	INFILES(1)='03021200.grid'
	OPEN(7,FILE=INFILES(1),STATUS='OLD',form='formatted')
	READ(7,*)(HDR(J),J=1,8)

	OPEN(11,FILE=F(2),STATUS='replace')
	WRITE(11,*)(HDR(J),J=1,8)
	OPEN(12,FILE=F(3),STATUS='replace')
...
C        PRINT*,'SCALE FOR INPUT?'
C        READ*,SCALE	

	OPEN(13+2*(nn-1),FILE=F(4+2*(nn-1)),STATUS='replace')
	WRITE(13+2*(nn-1),*)(HDR(J),J=1,8)
	OPEN(14+2*(nn-1),FILE=F(5+2*(nn-1)),STATUS='replace')
...
C *****************************************
	 ...
	 FC(I)=(1.458E-4)*SIN( PII*( HDR(3) - (I-1)*HDR(6) )/180. )
         ...
         write(6,*) i, HDR(3) - (I-1)*HDR(6), fc(i), DL, AP(i)
	...
	DO K=2,NW-1
 	...
 	END DO
        ...
        do 1000 ihalfday=1, nhalfday
           ...
           print *, ' nhalfday= ', nhalfday
           print *, ' ihalfday= ', ihalfday
...
c	 WRITE(6,*)K
	 DO I=1,HDRI(8)
  	  READ(7,91)(H(I,J,K),J=1,HDRI(7))
c  	  READ(7,111,END=2)(H(I,J,K),J=1,HDRI(7))
c111        format (17f7.1)
c  2	  DO J=1,HDRI(7)
...
c        print *,'psi(1,1,1) = ', psi(1,1,1), psi(1,2,1)       
  	...
  	DO K=1,NW
  	 DO I=1,HDRI(8)
    	  READ(7,91)(TH(I,J,K),J=1,HDRI(7))
c  	  READ(7,111,END=3)(TH(I,J,K),J=1,HDRI(7))
c  3	  DO J=1,HDRI(7)
...
c   **  the data is already in K ,  3/3/92  wu
	...
	END DO
  	...
  	DO K=1,NW
	 DO I=1,HDRI(8)
  	  READ(7,91)(U(I,J,K),J=1,HDRI(7))
c  	  READ(7,111,END=4)(U(I,J,K),J=1,HDRI(7))
c  4	  DO J=1,HDRI(7)
...
c	  END DO
	...
	END DO
    	...
    	DO K=1,NW
  	 DO I=1,HDRI(8)
  	  READ(7,91)(V(I,J,K),J=1,HDRI(7))
c  	  READ(7,111,END=5)(V(I,J,K),J=1,HDRI(7))
c  5	  DO J=1,HDRI(7)
...
c  **  calculate the boundary divergence term
       ...
       do i=1,HDRI(8)-1
          ...
          dsum=dsum +((u(i,HDRI(7),k)+u(i+1,HDRI(7),k))/2.)* DP
       End do

       do j=1,HdrI(7)-1
          dsum=dsum+((v(1,j,k)+v(1,j+1,k))/2.)* DL * AP(1)
          dsum=dsum-((v(HdrI(8),j,k)+v(HdrI(8),j+1,k))/2.)
     *                 *DL*AP(HdrI(8))
       End do

       dsum = dsum /( 2.*DP*(HdrI(8)-1)+ DL*(HDrI(7)-1)
     *               *(Ap(1)+Ap(HdrI(8)))  )
...
c  **  then integrate by Davis (2.40) to get the whole psi      
     ...
     *                           +        u(i+1,1,k) )/2.))* DP
       ...
       do j=1,HdrI(7)-1
          psi(HDRI(8),j+1,k)=psi(hdrI(8),j,k)+
     *    ( dsum+((v(HdrI(8),j,k)+v(HdrI(8),j+1,k))/2.) )
     *                 *DL*AP(HdrI(8))
       End do

       do i=HdrI(8),2,-1
          psi(i-1,HdrI(7),k)=psi(i,HdrI(7),k)
     *     +(dsum-((u(i,HdrI(7),k)+u(i-1,HdrI(7),k) )/2.))* DP
       End do
...
778    	DO I=1,HDRI(8)
	 WRITE(13+2*(ic-1),97)(THB(I,J),J=1,HDRI(7))
	END DO
	DO I=1,HDRI(8)
	 WRITE(13+2*(ic-1),97)(THT(I,J),J=1,HDRI(7))
	END DO
	DO K=2,NW-1
	 DO I=1,HDRI(8)
	  WRITE(13+2*(ic-1),97)(Q(I,J,K),J=1,HDRI(7)) 
	 END DO
	END DO
  97	FORMAT(45F10.2)
...
c  **  write out H and psi  , WU 9/30/91  **
	...
	DO K=1,NW
	 DO I=1,HDRI(8)
	  WRITE(14+2*(ic-1),97)(H(I,J,K),J=1,HDRI(7)) 
	 END DO
	END DO
...
c  ** to scale psi to fit H
	  ...
	  WRITE(14+2*(ic-1),97)(psi(I,J,K)/1.e5,J=1,HDRI(7)) 
	 END DO                          
	END DO
...
c  ** write out u and v and theta
 	DO K=1,NW
	 DO I=1,HDRI(8)
	  WRITE(14+2*(ic-1),97)(U(I,J,K),J=1,HDRI(7)) 
	 END DO                          
	END DO
 	DO K=1,NW
	 DO I=1,HDRI(8)
	  WRITE(14+2*(ic-1),97)(V(I,J,K),J=1,HDRI(7)) 
	 END DO                          
	END DO
 	DO K=1,NW
	 DO I=1,HDRI(8)
	  WRITE(14+2*(ic-1),97)(TH(I,J,K),J=1,HDRI(7)) 
	 END DO                          
	END DO
...
C****** WRITE OUT "BOUNDARY" THETA AND PV ****************
C        Write( str_format, "(i2,A5)" ) HDRI(7),"F10.2"
C	print*, str_format
C 98	FORMAT('str_format')
C        Write( str_format, "(i2,A5)" ) HDRI(7),"F10.2"
C	print*, str_format
	DO I=1,HDRI(8)
	 WRITE(11,97)(THBm(I,J)/nhalfday,J=1,HDRI(7))
	END DO
	DO I=1,HDRI(8)
	 WRITE(11,97)(THTm(I,J)/nhalfday,J=1,HDRI(7))
	END DO
	DO K=2,NW-1
	 DO I=1,HDRI(8)
	  WRITE(11,97)(Qm(I,J,K)/nhalfday,J=1,HDRI(7)) 
	 END DO
	END DO
...
c  **  write out H and psi  , WU 9/30/91  **
	...
	DO K=1,NW
	 DO I=1,HDRI(8)
	  WRITE(12,97)(Hm(I,J,K)/nhalfday,J=1,HDRI(7)) 
	 END DO
	END DO
...
c  ** to scale psi to fit H
	  ...
	  WRITE(12,97)(psim(I,J,K)/(1.e5*nhalfday),J=1,HDRI(7)) 
	 END DO
	END DO
