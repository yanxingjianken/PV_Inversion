...
def _process(name, gf):
    ...
    PSI = invert_vorticity_balanced(VOR, H, U, V, omega=1.75, max_iter=50000, tol=1e1)
...
np.savez(PY_OUT / "pass_ab.npz", Q_mean=mean["Q"], PSI_mean=mean["PSI"], VOR_mean=mean["VOR"],
         H_mean=mean["H"], TH_mean=mean["TH"],
         Q_event=event["Q"], PSI_event=event["PSI"], VOR_event=event["VOR"],
         H_event=event["H"], TH_event=event["TH"])
...
# ---- Plot 3: Mean ψ @ 500 hPa (matches Fortran contourf style) ----
...
psi = mean["PSI"][k500]  # physical ψ [m²/s] — same units as Fortran plot
cf = ax.contourf(LON2D, LAT2D, psi, cmap="RdBu_r", transform=pc, levels=20)
ax.contour(LON2D, LAT2D, psi, colors="black", linewidths=0.4, transform=pc,
           levels=np.linspace(psi.min(), psi.max(), 12))
plt.colorbar(cf, ax=ax, shrink=0.7, label="ψ [m²/s]")
ax.set_title(f"Wu Python — Pass A: ψ @ 500 hPa  [{psi.min():.0f}, {psi.max():.0f}] m²/s", fontsize=11, fontweight="bold")
