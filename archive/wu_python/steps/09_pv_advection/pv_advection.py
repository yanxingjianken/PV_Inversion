#!/usr/bin/env python3
"""Wu Python — Step 09: PV advection by piecewise-induced winds (pure Python)."""
...
print("=" * 60)
print("Wu Python — Step 09: PV advection (pure Python)")
print("=" * 60)

# Load Q from Pass A/B
ab = np.load(PY_OUT / "pass_ab.npz")
q_event = ab["Q_event"][6]  # k=6 = 250 hPa
q_mean  = ab["Q_mean"][6]
...
# Load piecewise PSI from Python Pass D
NW, block, NPIECES = 10, NY*NX, 3
_pass_d = np.load(PY_OUT / "pass_d.npz")
PSI = _pass_d["PSI_pieces"][:, 7, :, :]   # k=7 = 250 hPa
print(f"  PSI pieces loaded: shape={PSI.shape}")
