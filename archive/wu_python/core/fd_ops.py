...
def d_dpi(field_3d: np.ndarray) -> np.ndarray:
    ...
    Uses raw PI_VALS (= CP*(p/p0)^κ, NOT PI_WU) in the denominator to match
    pvpialln_94UV.f line ~465: STB=(TH(K+1)-TH(K-1))/(PI(K+1)-PI(K-1))
    where PI is the raw Exner function (not DPI-scaled).
    ...
    Returns:
        ∂field/∂π: (NW, NY, NX)
    """
    from wu_python.core.nondim import PI_VALS

    NW = field_3d.shape[0]
    out = np.zeros_like(field_3d)

    # Interior: centered differences using raw Exner PI_VALS
    for k in range(1, NW - 1):
        dpi = PI_VALS[k + 1] - PI_VALS[k - 1]
        out[k] = (field_3d[k + 1] - field_3d[k - 1]) / dpi

    # Boundaries: one-sided
    dpi = PI_VALS[1] - PI_VALS[0]
    out[0] = (field_3d[1] - field_3d[0]) / dpi
    dpi = PI_VALS[-1] - PI_VALS[-2]
