...
for _pname, _pinfo in _pieces.items():
    _piece_qlv[_pname] = _pinfo["levels"]  # list of Fortran 1-indexed K

_qlv_lower = _piece_qlv["lower"]
_qlv_middle = _piece_qlv["middle"]
_qlv_upper = _piece_qlv["upper"]

# HOUT and SIOUT: output all 10 levels
_hout_list = ",".join(str(i) for i in range(1, 11))
...
10,{_hout_list}
3
{len(_qlv_lower)},{','.join(str(k) for k in _qlv_lower)}
{_pd['ibc']}
{len(_qlv_middle)},{','.join(str(k) for k in _qlv_middle)}
{_pd['ibc']}
{len(_qlv_upper)},{','.join(str(k) for k in _qlv_upper)}
{_pd['ibc']}
"""
...
piece_titles = ["(a) Lower: 1000–700 hPa", "(b) Middle: 600–500 hPa",
                "(c) Upper: 400–200 hPa"]
